...
(
	...
	FROM	sys.objects 
	WHERE	type = 'P' 
			AND	name = 'SP_I_InsertScoreForParticipantInSchedule' 
			AND schema_name(schema_id) = 'dbo'
)
BEGIN
	DROP  PROCEDURE  SP_I_InsertScoreForParticipantInSchedule
END
GO

/*************************************************************************
* Name				: SP_I_InsertScoreForParticipantInSchedule
* Author			: Ramani Ganason
* Create date		: 30-Aug-2015
* Description		: Insert Score for Participant
*************************************************************************/
CREATE PROCEDURE [SP_I_InsertScoreForParticipantInSchedule]
(
...
/****************************************************************************************************
* If ScoreInParticipantInSchedule with same ScheduleID and ParticipantInScheduleID exist in DB, then set the old record to INACTIVE by default.
*****************************************************************************************************/
UPDATE T_ScoreInParticipantInSchedule
	SET ParticipantInScheduleID = null
	WHERE ScheduleID = @n_ScheduleID
	AND ParticipantInScheduleID = @n_ParticipantInScheduleID
...
GO
/*
GRANT EXEC ON [dbo].[SP_I_InsertScoreForParticipantInSchedule] TO USER
GO*/